<?php
$a = 0; 
$reqvaisseaudanslaflotte = $bd->prepare("SELECT idvaisseau, nomvaisseau, HPmaxvaisseau, HPvaisseau   
                FROM c_vaisseau WHERE idflottevaisseau = ?") ;
...
while ($repvaisseaudanslaflotte = $reqvaisseaudanslaflotte ->fetch())
	...
	echo '<a href="../conception_vaisseau/vaisseau.php?id=' . $repvaisseaudanslaflotte['idvaisseau'] . '">' . $repvaisseaudanslaflotte['nomvaisseau'] . '</a> (' . number_format($PourcentHP, 0) . '%)'; 
