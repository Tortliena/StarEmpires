...
elseif ($_POST['supprimer'] == 'non') 
	...
	$reqbiens->execute(array($_POST['idplanete'])); 
	...
	$reqcompterpop  = $bdg->prepare('SELECT COUNT(*) as pop FROM population WHERE idplanetepop = ? AND typepop = ? AND typepoparrivee = "rien"'); 
	...
	if (in_array($_POST['poparrivee'], array(2,3))) // permet de limiter aux pop avec des limites. 
		...
		if ($repcompterdestination['pop'] + $repcompterencoursdetransfo['pop'] + $_POST['combien'] > $replimitepop[0]) 
			...
			header("Location: ../planete.php?message=30" . urlencode($_POST['idplanete']); 
		    exit(); 
			}	 
		} 
	*/ 
